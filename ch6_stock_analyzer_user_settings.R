# BUSINESS SCIENCE ----
# DS4B 202-R ----
# STOCK ANALYZER APP - CHALLENGE 6 -----
# Version 1

# APPLICATION DESCRIPTION ----
# - Connect Moving Average Settings
# - Add time window with min 180 days & max 730 days
...
server <- function(input, output, session) {
    ...
    observe({
        if (credentials()$user_auth) {
            ...
            reactive_values$mavg_short     <- user_data_tbl %>% pull(user_settings) %>% pluck(1) %>% pull(mavg_short)
            reactive_values$mavg_long      <- user_data_tbl %>% pull(user_settings) %>% pluck(1) %>% pull(mavg_long)
            reactive_values$time_window    <- user_data_tbl %>% pull(user_settings) %>% pluck(1) %>% pull(time_window)
        }
    })
                        ...
                        wellPanel(
                            ...
                            div(
                                ...
                                sliderInput(inputId = "time_window",
                                            ...
                                            value = reactive_values$time_window, 
                                            min   = 180,
                                            max   = 730),
                                actionButton(inputId = "apply_and_save", 
                                             label   = "Apply & Save", 
                                             icon    = icon("save"))
                            ) %>% hidden()
                        )
